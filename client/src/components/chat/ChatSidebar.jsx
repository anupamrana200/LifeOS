...
export const ChatSidebar = ({ conversations, isLoading, isOpen, onClose, onDelete, onNewChat, onSelect, selectedConversationId }) => {
  ...
  return (
    <aside aria-label="Conversations" className={cn('fixed inset-y-0 left-0 z-modal flex min-h-0 w-[min(18rem,calc(100vw-1rem))] -translate-x-full flex-col border-r border-border bg-card transition-transform duration-theme lg:static lg:z-base lg:w-auto lg:translate-x-0', isOpen && 'translate-x-0')}>
